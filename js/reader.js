...
(function () {
    ...
    function addSwipePagination() {
        ...
        function handleTouchEnd(event) {
            ...
            // 判断是否为有效滑动
            if (swipeDirection === 'horizontal' && 
                (Math.abs(diffX) > SWIPE_THRESHOLD || velocity > SWIPE_VELOCITY)) {
                
                viewport.classList.add('swipe-transition');
                
                if (diffX > 0) {
                    // 向左滑动 - 下一页
                    viewport.classList.add('swipe-left');
                    setTimeout(() => {
                        goNextPage();
                        viewport.classList.remove('swipe-left', 'swipe-transition');
                    }, 250);
                } else {
                    // 向右滑动 - 上一页
                    viewport.classList.add('swipe-right');
                    setTimeout(() => {
                        goPrevPage();
                        viewport.classList.remove('swipe-right', 'swipe-transition');
                    }, 250);
                }
            }
        ...
        // 为iframe内容也绑定触摸事件（解决iframe拦截触摸的问题）
        function bindIframeTouchEvents() {
        ...
        // 监听iframe加载完成后绑定事件
        if (rendition) {
            rendition.on('rendered', bindIframeTouchEvents);
        }

        // 初始绑定
        setTimeout(bindIframeTouchEvents, 500);
    }
